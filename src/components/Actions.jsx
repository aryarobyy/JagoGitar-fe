...
const Actions = ({ post }) => {
	...
	const handleReply = async () => {
		...
		setIsReplying(true);
		try {
			const data = await createComment({ text: reply, postId: post._id });
			if (data.error) return showToast("Error", data.error, "error");
	
			const updatedPosts = posts.map((p) => {
				if (p._id === post._id) {
					const replies = Array.isArray(p.replies) ? p.replies : [];
				return { ...p, replies: [...replies, data] };
				}
				return p;
			});
			setPosts(updatedPosts);
...
// 		  <Comment key={comment._id} commentId={comment._id} commentData={comment} />
// 		))}
